...
local function set_mappings(start_buf, bufs, type)
    ...
    for _, b in pairs(bufs) do
        api.nvim_buf_set_keymap(
            ...
            "n",
            "k",
            ':lua require("springboot-nvim.ui.springboot_nvim_ui").navigate_to_package()<cr>',
        ...
        if type == "class" then
            api.nvim_buf_set_keymap(
                ...
                "n",
                "j",
                ':lua require("springboot-nvim.ui.springboot_nvim_ui").navigate_to_class()<cr>',
                { nowait = true, noremap = true, silent = true }
            )
            api.nvim_buf_set_keymap(
                b,
                "n",
                "<cr>",
                ':lua require("springboot-nvim.utils").generate_java_file('
                    .. start_buf
                    .. ',"'
                    .. type
                    .. '","'
        ...
        if type == "record" then
            api.nvim_buf_set_keymap(
                ...
                "n",
                "j",
                ':lua require("springboot-nvim.ui.springboot_nvim_ui").navigate_to_record()<cr>',
                { nowait = true, noremap = true, silent = true }
            )
            api.nvim_buf_set_keymap(
                b,
        ...
        if type == "interface" then
            api.nvim_buf_set_keymap(
                ...
                "n",
                "j",
                ':lua require("springboot-nvim.ui.springboot_nvim_ui").navigate_to_interface()<cr>',
                { nowait = true, noremap = true, silent = true }
            )
            api.nvim_buf_set_keymap(
                b,
        ...
        if type == "enum" then
            api.nvim_buf_set_keymap(
                ...
                "n",
                "j",
                ':lua require("springboot-nvim.ui.springboot_nvim_ui").navigate_to_enum()<cr>',
                { nowait = true, noremap = true, silent = true }
            )
            api.nvim_buf_set_keymap(
                b,
        ...
        api.nvim_buf_set_keymap(
            ...
            "n",
            "q",
            ':lua require("springboot-nvim.ui.springboot_nvim_ui").close_ui()<cr>',
            { nowait = true, noremap = true, silent = true }
        )
...
local function create_generate_class_ui()
    ...
    local package_components = ui_utils.draw_labeled_input(
        46,
        1,
        row + 1,
        col + 1,
        "Package: ",
        popup_components.popup_buf,
        ui_utils.package_text(main_class)
    )
    local class_components =
        ui_utils.draw_labeled_input(46, 1, row + 4, col + 1, "Class: ", popup_components.popup_buf, "")
    components = {
        bufs = {
    ...
    api.nvim_buf_set_lines(
        ...
        -1,
        false,
        { edit_package .. string.rep(" ", 60 - (string.len(edit_package) + string.len(edit_class))) .. edit_class }
    )
    api.nvim_buf_set_lines(
        popup_components.popup_buf,
        row + 9,
        -1,
        false,
        { confirm_class .. string.rep(" ", 60 - (string.len(confirm_class) + string.len(close_menu))) .. close_menu }
    )
    api.nvim_set_current_win(components.wins.package_win)
...
local function create_generate_record_ui()
    ...
    local record_components =
        ui_utils.draw_labeled_input(46, 1, row + 4, col + 1, "Record: ", popup_components.popup_buf, "")
    components = {
        bufs = {
            popup_buf = popup_components.popup_buf,
            border_buf = popup_components.border_buf,
...
local function create_generate_interface_ui()
    ...
    local interface_components =
        ui_utils.draw_labeled_input(46, 1, row + 4, col + 1, "Interface: ", popup_components.popup_buf, "")
    components = {
        bufs = {
            popup_buf = popup_components.popup_buf,
            border_buf = popup_components.border_buf,
...
local function create_generate_enum_ui()
    ...
    local enum_components =
        ui_utils.draw_labeled_input(46, 1, row + 4, col + 1, "Enum: ", popup_components.popup_buf, "")
    components = {
        bufs = {
            popup_buf = popup_components.popup_buf,
            border_buf = popup_components.border_buf,
